...
CREATE TABLE Employee (
    ...
    DepartmentName VARCHAR(10) NOT NULL,
    BossNumber SMALLINT UNSIGNED NOT NULL,
...
(1, 'Alice', 75000, 'Management', 1),  -- Alice is the boss of the company, so her boss number is her own employee number, this gets around the problem of having a NULL value for the boss number of the boss of the company.
...
WHERE Delivery.SupplierNumber = Supplier.SupplierNumber GROUP BY Supplier.SupplierNumber, 
Supplier.SupplierName
HAVING COUNT(DISTINCT Delivery.ItemName) >= 10;

--8
SELECT Boss.EmployeeNumber,
Boss.EmployeeName AS 'Boss Name', COUNT(*) AS
'Employees'
...
--reordered, filter by item type first, then join to supplier and item tables

SELECT Delivery.SupplierNumber,
SupplierName, Delivery.ItemName,
AVG(Delivery.DeliveryQuantity) AS
'Average Quantity'
FROM ((Item NATURAL JOIN Delivery) NATURAL JOIN Supplier)
WHERE Item.ItemType = 'N'
GROUP BY Delivery.SupplierNumber,
SupplierName, Delivery.ItemName ORDER BY
Delivery.SupplierNumber, SupplierName,
...
--Q6. Find the suppliers that deliver compasses and at least one other kind of item
...
--Q7. Find the suppliers that deliver compasses and at least three other kinds of item
SELECT DISTINCT Delivery.SupplierNumber,
Supplier.SupplierName FROM (Supplier
